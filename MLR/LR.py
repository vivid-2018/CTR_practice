# encoding = utf8
"""
This is an implementation of LR model with Tensorflow.
@author:vivid.
"""
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            for i in range(len(self.features)):
                s = self.features[i]
                this_vec = tf.gather(self.weights[s+'_embedding'],self.input[:,i])
                lr_vec.append(this_vec)

            self.out = sum(lr_vec)
            self.out = tf.add(tf.reshape(self.out, (-1, 1)), self.weights['bias'])
