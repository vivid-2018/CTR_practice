...
class FM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.fm_out = tf.concat(fm_result,axis=1)
            self.fm_out = tf.reshape(self.fm_out,[-1,len(self.features), self.embedding_size])
            self.fm_out = tf.square(tf.reduce_sum(self.fm_out,axis=1)) - tf.reduce_sum(tf.square(self.fm_out),axis=1)
            self.fm_out = 0.5 * tf.reduce_sum(self.fm_out, axis=1, keepdims=True)
