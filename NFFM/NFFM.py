# encoding = utf8
'''
This is an implementation of NFFM model with Tensorflow.
@author:vivid.
'''
...
class NFFM(BaseEstimator, TransformerMixin):

    def __init__(self, features, feature_size, feature_field, embedding_size=16, learning_rate=0.001,
                 layers=[200, 200, 200], batch_norm=True, dropout=0.7, l2_reg=0,
        ...
        self.field_num = len(set(self.feature_field.values()))
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            for i in range(len(self.features)):
                s1 = self.features[i]
                fm_vec1 = tf.gather(self.weights[s1+'_fm_embedding'],self.input[:,i])
                fm_vec1 = tf.reshape(fm_vec1, (-1, self.field_num, self.embedding_size))
                for j in range(i+1,len(self.features)):
                    s2 = self.features[j]
                    fm_vec2 = tf.gather(self.weights[s2+'_fm_embedding'],self.input[:,j])
                    fm_vec2 = tf.reshape(fm_vec2, (-1, self.field_num, self.embedding_size))

                    result.append(tf.multiply(fm_vec1[:,j,:], fm_vec2[:,i,:]))

            deep_part = tf.concat(result, axis=1)
            ...
            if self.l2_reg > 0:
                ...
                for i in range(len(self.layers)):
                    self.loss += tf.contrib.layers.l2_regularizer(self.l2_reg)(self.weights['weight_%d' % i])
                self.loss += tf.contrib.layers.l2_regularizer(self.l2_reg)(self.weights['project_weight'])
    ...
    def _init_weights(self):
        ...
        for s in self.feature_size:
            ...
            weights[s+'_fm_embedding'] = tf.Variable(
                tf.random_normal([n_value,self.embedding_size*self.field_num],0.0,0.01),
        ...
        n = self.field_num * (self.field_num - 1) // 2
        for i in range(len(self.layers)):
            input_size = self.layers[i-1] if i > 0 else self.embedding_size * n
