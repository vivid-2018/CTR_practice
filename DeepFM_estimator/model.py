...
def init_weights(params):
    ...
    for i in range(len(layers)):
        input_size = layers[i-1] if i > 0 else len(args.CATEGORICAL_FEATURES)*embedding_size
...
def creat_graph(features, labels, mode, params):
    ...
    fm_wide = tf.concat(fm_embedding_vecs, axis=1)
    fm_wide = tf.reshape(fm_wide, (-1,args.embedding_size, len(args.CATEGORICAL_FEATURES)))
