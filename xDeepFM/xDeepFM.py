...
# encoding = utf8
"""
This is an implementation of xDeepFM model with Tensorflow.
@author:vivid.
"""
...
class xDeepFM(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, features, feature_size, embedding_size=16, learning_rate=0.001,
                 layer_size=[128, 128], l2_reg=0.00001,
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            for i in range(len(self.features)):
                s = self.features[i]
                lr_vec = tf.gather(self.weights[s+'_lr'],self.input[:,i])
                fm_vec = tf.gather(self.weights[s+'_embedding'],self.input[:,i])
                lr_result.append(lr_vec)
                fm_result.append(fm_vec)

            # lr part
            lr_part = tf.add(sum(lr_result), self.weights['bias_lr'])

            # wide part
            wide_input = tf.reshape(tf.concat(fm_result,axis=1), (-1, len(self.features), self.embedding_size))
            wide_layers = [wide_input]
            ...
            for i in range(len(self.layer_size)):
                ...
                z_k = tf.nn.conv1d(z_k, filters=self.weights['filter_%d' % i], stride=1, padding='VALID')
                z_k = tf.add(z_k, self.weights['filter_bias_%d' % i])
                z_k = tf.nn.relu(z_k)
                wide_layers.append(tf.transpose(z_k, [0, 2, 1]))

            wide_part = tf.concat([tf.reduce_sum(item,axis=2) for item in wide_layers], axis=1)
    ...
    def _init_weights(self):
        ...
        # 投影权重&偏置
        input_size = self.layers[-1] + len(self.features) + sum(self.layer_size) + 1
        output_size = 1
        glorot = np.sqrt(2.0 / (input_size + output_size))
        ...
        # wide 部分权重
        for i in range(len(self.layer_size)):
            h_k_1 = self.layer_size[i-1] if i > 0 else len(self.features)
            out_channels = self.layer_size[i]
            in_channels = len(self.features) * h_k_1
            glorot = np.sqrt(2.0 / (in_channels + out_channels))
            weights['filter_%d' % i] = tf.Variable(
                tf.random_normal([1, in_channels, out_channels], 0.0, glorot),
                name='filter_%d' % i
            )
            weights['filter_bias_%d' % i] = tf.Variable(
                tf.random_uniform([1,1,out_channels], 0.0, 1.0),
                name='filter_bias_%d' % i
            )
